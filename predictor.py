...
def eval_dataset_full(net, out_channels, loader, class_weights=None, save_dir=None):
    ...
    for i, (batch_data, batch_label, _) in enumerate(loader):
        if i == 0:
...
def predict_images(net, args, dst_wh=(512, 512), save_dir=None, num_classes=2):
    ...
    for path in paths:
        ...
        dst_frame = cv2.resize(frame, dst_wh)
        dst_prediction = cv2.resize(prediction_np, dst_wh)
